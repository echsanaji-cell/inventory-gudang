...
@app.route('/api/pembagian-buku/cari-suggest')
@login_required
@viewer_blocked
def api_cari_suggest_pembagian():
    q = request.args.get('q', '').strip()
    if len(q) < 2:
        return jsonify([])
    restriksi_user = session.get('area_restriction')
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    query = """SELECT penerbit, isbn, MAX(judul) as judul
               FROM pembagian_buku_master
               WHERE (judul ILIKE %s OR isbn ILIKE %s OR penerbit ILIKE %s)"""
    params = [f'%{q}%', f'%{q}%', f'%{q}%']
    if restriksi_user:
        query += " AND warna_area = %s"
        params.append(restriksi_user)
    query += " GROUP BY penerbit, isbn ORDER BY MAX(judul) ASC LIMIT 15"

    cur.execute(query, tuple(params))
    hasil = cur.fetchall()
    cur.close()
    conn.close()
    return jsonify([{'penerbit': r['penerbit'], 'isbn': r['isbn'], 'judul': r['judul'],
                      'label': f"{r['judul']} ({r['isbn']}) — {r['penerbit'] or '(Tanpa Penerbit)'}"} for r in hasil])
...
def pembagian_buku_list():
    ...
    query = """
        SELECT penerbit, isbn, MAX(judul) as judul, MAX(pengarang) as pengarang,
    ...
    count_query = "SELECT COUNT(DISTINCT isbn) as total FROM pembagian_buku_master WHERE 1=1"
    count_params = []
    if restriksi_user:
        count_query += " AND warna_area = %s"
        count_params.append(restriksi_user)
    cur.execute(count_query, tuple(count_params))
    total_judul_keseluruhan = cur.fetchone()['total']
    ...
    return render_template(
        ...
        daftar_judul=daftar_judul, search=search,
        page=page, total_halaman=total_halaman, total_data=total_data,
        total_judul_keseluruhan=total_judul_keseluruhan,
        restriksi_user=restriksi_user
    )
...
def pembagian_buku_rekap_list():
    ...
    query = f"""
        SELECT nama_perpustakaan, kabupaten_kota, provinsi, no_box, warna_area,
               COUNT(*) as total_judul,
               COALESCE(SUM(eksemplar), 0) as total_eksemplar,
               COALESCE(SUM(CASE WHEN sudah_diambil THEN 0 ELSE eksemplar END), 0) as sisa_eksemplar,
    ...
    count_query = "SELECT COUNT(DISTINCT (nama_perpustakaan, kabupaten_kota, provinsi, no_box)) as total FROM pembagian_buku_master WHERE 1=1"
    count_params = []
    if restriksi_user:
        count_query += " AND warna_area = %s"
        count_params.append(restriksi_user)
...
def pembagian_buku_rekap_detail():
    ...
    no_box_tampil = daftar_buku[0]['no_box']
    ...
    return render_template(
        ...
        nama=nama, kabupaten=kabupaten, provinsi=provinsi, no_box=no_box_tampil, warna_area=warna_area,
...
def api_cari_suggest_pembagian_perpustakaan():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    query = """SELECT nama_perpustakaan, kabupaten_kota, provinsi, no_box
               FROM pembagian_buku_master
    ...
    if restriksi_user:
        query += " AND warna_area = %s"
        params.append(restriksi_user)
    query += " GROUP BY nama_perpustakaan, kabupaten_kota, provinsi, no_box ORDER BY nama_perpustakaan ASC LIMIT 15"
    ...
    return jsonify([{'nama': r['nama_perpustakaan'], 'kabupaten': r['kabupaten_kota'], 'provinsi': r['provinsi'],
                      'label': f"{r['nama_perpustakaan']} ({r['kabupaten_kota']}, Box {r['no_box'] if r['no_box'] is not None else '-'})"} for r in hasil])
...
def pembagian_buku_dashboard():
    ...
    cur.execute("""
        SELECT COUNT(DISTINCT (penerbit, isbn)) as total_judul,
               COUNT(DISTINCT (nama_perpustakaan, kabupaten_kota, provinsi, no_box)) as total_perpustakaan,
               COALESCE(SUM(eksemplar), 0) as total_eksemplar,
    ...
    cur.execute("""
        SELECT provinsi,
               COALESCE(SUM(eksemplar), 0) as total_eksemplar,
               COALESCE(SUM(CASE WHEN sudah_diambil THEN eksemplar ELSE 0 END), 0) as eksemplar_selesai,
               COUNT(DISTINCT (nama_perpustakaan, kabupaten_kota, no_box)) as total_perpustakaan
        FROM pembagian_buku_master
    ...
    cur.execute("""
        SELECT nama_perpustakaan, kabupaten_kota, provinsi, no_box, warna_area,
               COALESCE(SUM(eksemplar), 0) as total_eksemplar
        FROM pembagian_buku_master
        GROUP BY nama_perpustakaan, kabupaten_kota, provinsi, no_box, warna_area
    ...
    cur.execute("""
        SELECT COUNT(*) as total FROM (
            SELECT nama_perpustakaan, kabupaten_kota, provinsi, no_box
            FROM pembagian_buku_master
            GROUP BY nama_perpustakaan, kabupaten_kota, provinsi, no_box
            HAVING COALESCE(SUM(CASE WHEN sudah_diambil THEN eksemplar ELSE 0 END), 0) = 0
        ) sub
    """)
...
@app.route('/buku/<int:buku_id>/detail-distribusi/kirim-massal', methods=['POST'])
@login_required
@viewer_blocked
def buku_detail_distribusi_kirim_massal(buku_id):
    data = request.get_json(silent=True) or {}
    tujuan_ids = data.get('tujuan_ids', [])

    if not tujuan_ids:
        return jsonify({'success': False, 'message': 'Pilih minimal 1 tujuan.'}), 400
    ...
    rincian = []
    total_kebutuhan = 0
    for tujuan_id in tujuan_ids:
        cur.execute(
            """SELECT dr.jumlah_rencana,
                      COALESCE((SELECT SUM(tr.jumlah) FROM transaksi tr
                                WHERE tr.tujuan_id = dr.tujuan_id AND tr.buku_id = dr.buku_id AND tr.tipe = 'keluar'), 0) as jumlah_terkirim
               FROM distribusi_rencana dr
               WHERE dr.tujuan_id = %s AND dr.buku_id = %s""",
            (tujuan_id, buku_id)
        )
        info = cur.fetchone()
        if not info:
            continue
        sisa = info['jumlah_rencana'] - info['jumlah_terkirim']
        if sisa <= 0:
            continue
        rincian.append((tujuan_id, sisa))
        total_kebutuhan += sisa

    if not rincian:
        cur.close()
        conn.close()
        return jsonify({'success': False, 'message': 'Tidak ada tujuan yang perlu dikirim (mungkin sudah lengkap semua).'}), 400

    if buku['stok'] < total_kebutuhan:
        cur.close()
        conn.close()
        return jsonify({'success': False, 'message': f'Stok tidak cukup. Dibutuhkan {total_kebutuhan}, tersedia {buku["stok"]}.'}), 400

    try:
        for tujuan_id, sisa in rincian:
            cur.execute(
                """INSERT INTO transaksi (buku_id, tipe, jumlah, user_id, tanggal, tujuan_id, keterangan)
                   VALUES (%s, 'keluar', %s, %s, CURRENT_DATE, %s, 'Kirim dari Detail Distribusi (Checklist)')""",
                (buku_id, sisa, session['user_id'], tujuan_id)
            )
        cur.execute("UPDATE buku SET stok = stok - %s, updated_at = NOW() WHERE id = %s", (total_kebutuhan, buku_id))
        conn.commit()

        catat_aktivitas(
            'Kirim Massal dari Detail Distribusi',
            f'Buku "{buku["judul"]}" dikirim ke {len(rincian)} tujuan sekaligus, total {total_kebutuhan} eks'
        )

        cur.execute("SELECT stok FROM buku WHERE id = %s", (buku_id,))
        stok_baru = cur.fetchone()['stok']

        return jsonify({'success': True, 'jumlah_tujuan': len(rincian), 'total_dikirim': total_kebutuhan, 'stok_baru': stok_baru})
    except Exception:
        conn.rollback()
        return jsonify({'success': False, 'message': 'Gagal menyimpan pengiriman massal.'}), 500
    finally:
        cur.close()
        conn.close()
